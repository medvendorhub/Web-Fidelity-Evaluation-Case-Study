/**
 * Candidate B — close replication with controlled subtle defects.
 *
 * Defects (intentional, documented in assessment findings):
 * 1. All headings use font-weight: 400 (normal) instead of the reference's 600/700.
 * 2. Doctor cards grid uses gap-3 (12px) instead of the reference's gap-6 (24px).
 * 3. The booking-form search button is icon-only with NO aria-label (WCAG 2.1 SC 4.1.2 failure).
 * 4. At tablet breakpoint (~768px), the third doctor card wraps to its own row due
 *    to a slightly incorrect grid definition (col-span logic breaks at that width).
 *
 * Everything else matches the reference as closely as possible.
 */
  ...
  return (
    <>
      <DeviceBar
        active={device}
        onChange={setDevice}
        label="Candidate B — Healthcare Appointment Form"
      />
      <DeviceViewport width={device}>
        <div className="font-sans text-slate-900 antialiased">
          ...
          <header className="bg-white border-b border-slate-200 sticky top-0 z-40" role="banner">
            <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
              <a href="#" className="flex items-center gap-2 text-indigo-700 font-bold text-xl tracking-tight" aria-label="MediCare home">
                <Hospital size={26} />
                {/* DEFECT: brand name uses font-weight: 400 because heading weight bleeds through */}
                <span className="font-normal">MediCare</span>
              </a>
              ...
              <div className="flex items-center gap-3">
                <a href="tel:18005551234" className="hidden md:flex items-center gap-1.5 text-sm text-slate-600 hover:text-indigo-600 transition-colors">
                  <Phone size={14} />
                  1-800-555-1234
                </a>
            ...
            {menuOpen && (
              <nav id="mobile-nav-b" className="md:hidden border-t border-slate-200 bg-white px-6 py-4 space-y-3 text-sm font-medium text-slate-700" aria-label="Mobile primary">
          ...
          {/* ── Hero ── */}
          <section className="bg-indigo-900 text-white py-20 px-6" aria-labelledby="hero-b-heading">
            <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-12 items-center">
              <div className="space-y-6">
                ...
                {/* DEFECT: heading uses font-weight: 400 (normal) instead of reference's 700 */}
                <h1 id="hero-b-heading" className="text-4xl md:text-5xl font-normal tracking-tight leading-tight">
          ...
          {/* ── Feature strip ── (matches reference) */}
            ...
            <div className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-3 gap-6">
              {features.map(({ Icon, title, desc }) => (
                <div key={title} className="flex items-start gap-4 text-white">
                  ...
                  <div>
                    {/* DEFECT: section titles also affected by font-weight: 400 */}
          ...
          {/* ── Booking form ── */}
          <section id="booking" className="py-16 px-6 bg-slate-50" aria-labelledby="booking-b-heading">
            <div className="max-w-4xl mx-auto">
              {/* DEFECT: heading font-weight: 400 */}
              <h2 id="booking-b-heading" className="text-2xl font-normal text-slate-900 mb-2">Quick Appointment Booking</h2>
                ...
                <form onSubmit={e => e.preventDefault()} noValidate>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="flex flex-col gap-1.5">
                      <label htmlFor="b-name" className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Patient Name</label>
                      <div className="flex items-center gap-2 border border-slate-300 rounded-md px-3 py-2.5 bg-white focus-within:ring-2 focus-within:ring-indigo-500 focus-within:border-indigo-500 transition-shadow">
                        ...
                        <input id="b-name" type="text" placeholder="Full name" className="bg-transparent outline-none w-full text-sm text-slate-900 placeholder:text-slate-400" />
                      </div>
                    </div>
                    <div className="flex flex-col gap-1.5">
                      <label htmlFor="b-date" className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Preferred Date</label>
                      <div className="flex items-center gap-2 border border-slate-300 rounded-md px-3 py-2.5 bg-white focus-within:ring-2 focus-within:ring-indigo-500 focus-within:border-indigo-500 transition-shadow">
                        <Calendar size={15} className="text-slate-400 shrink-0" />
                        <input id="b-date" type="date" className="bg-transparent outline-none w-full text-sm text-slate-900" />
                      </div>
                    </div>
                    <div className="flex flex-col gap-1.5">
                      <label htmlFor="b-dept" className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Department</label>
                      <div className="flex items-center gap-2 border border-slate-300 rounded-md px-3 py-2.5 bg-white focus-within:ring-2 focus-within:ring-indigo-500 focus-within:border-indigo-500 transition-shadow">
                        <Stethoscope size={15} className="text-slate-400 shrink-0" />
                        <select id="b-dept" className="bg-transparent outline-none w-full text-sm text-slate-900 appearance-none">
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      {/*
                        DEFECT: icon-only button — no aria-label, no visible text.
                        Fails WCAG 2.1 SC 4.1.2 (Name, Role, Value).
                        The reference renders <Search /> + "Search" text with an aria-label.
                      */}
                      <button
                        type="submit"
                        className="flex items-center justify-center bg-indigo-600 hover:bg-indigo-700 text-white rounded-md py-2.5 font-semibold text-sm transition-colors w-full"
                      >
                        <Search size={18} />
                      </button>
                    </div>
          ...
          {/* ── Doctors ── */}
          <section id="doctors" className="py-16 px-6 bg-white" aria-labelledby="doctors-b-heading">
            <div className="max-w-6xl mx-auto">
              <div className="flex items-end justify-between mb-10">
                <div>
                  {/* DEFECT: heading font-weight: 400 */}
                  <h2 id="doctors-b-heading" className="text-3xl font-normal text-slate-900 tracking-tight">Our Top Specialists</h2>
              ...
              {/*
                DEFECT: gap-3 (12px) instead of the reference's gap-6 (24px) — slightly too tight.
                DEFECT: grid definition causes the third card to wrap awkwardly at ~768px because
                the columns aren't properly constrained at the tablet breakpoint.
                Using sm:grid-cols-2 means at 768px exactly, we get 2 columns and the third card
                takes up an awkward half row.
              */}
                  ...
                  <article key={doc.name} className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <div className="p-5">
                      <div className="flex items-start justify-between mb-1">
                        {/* DEFECT: card heading also font-weight: 400 */}
                        <h3 className="text-base font-normal text-slate-900">{doc.name}</h3>
          ...
          {/* ── Testimonials ── (matches reference) */}
          <section className="py-16 px-6 bg-slate-50" aria-labelledby="testimonials-b-heading">
            <div className="max-w-6xl mx-auto">
              {/* DEFECT: heading font-weight: 400 */}
              <h2 id="testimonials-b-heading" className="text-3xl font-normal text-slate-900 tracking-tight mb-10">What our patients say</h2>
